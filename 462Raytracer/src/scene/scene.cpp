...
Ray Geometry::to_local(const Ray& r){
	Ray local_r;
	local_r.d = invMat.transform_vector(r.d);
	local_r.e = invMat.transform_point(r.e);
	return local_r;
}
...
BvhNode* Scene::gen_bvh_tree(){
	BvhNode* root = new BvhNode(geometries,0 , 0, geometries.size() - 1);
	return root;
}
