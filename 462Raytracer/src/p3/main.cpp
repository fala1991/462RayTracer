...
struct Options
{
    ...
    int width, height;
    int num_samples;
    RaytracerOptions raytracer_opt;
};
...
bool RaytracerApplication::initialize()
{
        ...
        for ( size_t i = 0; i < scene.num_meshes(); ++i )
    {
		...
		// load cube map
		if (options.skybox_filename){
			scene.skybox = new Cubemap(std::string(options.skybox_filename));
			scene.skybox->load();
		}
    }
...
void RaytracerApplication::handle_event( const SDL_Event& event )
{
    ...
    case SDL_KEYDOWN:
        ...
        case KEY_SEND_PHOTONS:
            raytracer.initialize(&scene, options.num_samples, 0, 0, options.raytracer_opt);
    ...
    case SDL_MOUSEBUTTONDOWN:
            if(event.button.button == SDL_BUTTON_RIGHT){
                ...
                get_dimension( &width, &height );
                toggle_raytracing(width,height);
                raytracer.trace_focus(event.button.x, event.button.y);
            }
        break;
    default:
        break;
...
void RaytracerApplication::toggle_raytracing( int width, int height )
{
    ...
    if ( !raytracing ) {
        ...
        if (!raytracer.initialize(&scene, options.num_samples, width, height, options.raytracer_opt))
        {
...
static bool parse_args( Options* opt, int argc, char* argv[] )
{
	...
	opt->skybox_filename = NULL;
    opt->open_window = true;
    opt->width = DEFAULT_WIDTH;
    opt->height = DEFAULT_HEIGHT;
    opt->num_samples = 1;
    opt->raytracer_opt.focus = 0;
    opt->raytracer_opt.gloss = 0;
    for (int i = 2; i < argc; i++)
    {
		...
		case 'm':
			if (i < argc - 1)
				opt->skybox_filename = argv[++i];
			break;
        case 'g':
            if (i < argc - 1)
                opt->raytracer_opt.gloss = atof(argv[++i]);
            break;
		default:
			break;
        }
